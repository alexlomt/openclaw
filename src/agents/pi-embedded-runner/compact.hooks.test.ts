...
describe("compactEmbeddedPiSession hooks (ownsCompaction engine)", () => {
  ...
  it("falls back to context-engine compaction when native harness compaction is unavailable", async () => {
    maybeCompactAgentHarnessSessionMock.mockResolvedValueOnce({
      ok: false,
      compacted: false,
      reason: "thread/compact/start failed: upstream returned HTTP 404",
      failure: { reason: "native_compaction_unavailable", status: 404 },
    });
